...
class InviteContact extends ConsumerStatefulWidget{
  const InviteContact({Key? key, }) : super(key: key);

  @override
  _InviteContactState createState() => _InviteContactState();
}

class _InviteContactState extends ConsumerState<InviteContact> {
...
//     setState(() {});
//   }
// ref.watch(viewgroupprov);
InviteDataResponse? inviteDataResponse;
 //getAllinviteGroupData
 getAllinviteGroupData1( InviteContactData prov,context)async { 
     inviteDataResponse = await GroupAndChatRepository().getAllinviteGroupData(groupid:ref.read(viewgroupprov).groupid!.toInt() ,touser:prov.selected );
   
    if(inviteDataResponse!.result==false){
       showToast(inviteDataResponse!.message,
     ...
     Navigator.pop(context);
      print(inviteDataResponse!.message); 
    }
     setState(() {
...
InviteContactDataResponseModle? allConatactResponse;

getCollectionData()async { 
     allConatactResponse = await GroupAndChatRepository().getinviteContactData(id: ref.read(viewgroupprov).groupid!.toInt() );
    ...
    ref.read(invitecontactProv).selectedIndex = selectionIndex;
    setState(() {});
...
uploadImageToDB(context,CreateGroupData groupdp,ContactData data) async {
       ...
       String filename= basename(groupdp.file!.path);
       print('object1to$filename');
        print('${groupdp.subject}');
        print('${groupdp.file}');
        print('${groupdp.message}');
         print('${"\"${"${data.selected.join(',')}"}\""}');
         var formData;
      try{
   formData = FormData.fromMap({
  'group_image':await MultipartFile.fromFile(groupdp.file!.path, filename: filename),
...
//       groupdp.filepathprov=dp.groupImageUrl;
         print('response data11 ${ groupdp1.status}');
//       print('response data ${dp.groupImageUrl}');
//        print('response data ${dp.id}');
    ...
    return AppBar(
      ...
      elevation: 1,
       actions: [
          MaterialButton(
            onPressed: (){
              //getAllinviteGroupData(ref.read())
             getAllinviteGroupData1(ref.read(invitecontactProv),context);
            },
            child: Center(
              child: Text('Done',
               style: TextStyle(
...
bool isSelected = false;
  ...
  onSelect(InviteContactData prov) {
...
print(prov.selected);
  ...
  @override
  Widget build(BuildContext context) {
    final provider=ref.watch(invitecontactProv);
...
//       );
        ...
        // onTap: (){
        //    Navigator.push(context, MaterialPageRoute(builder: (context) => PersonalInviteContact()));
