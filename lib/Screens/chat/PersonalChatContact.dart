...
class PersonalChatContact extends ConsumerStatefulWidget {
  const PersonalChatContact({Key? key}) : super(key: key);

  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _PersonalChatContactState();
}

class _PersonalChatContactState extends ConsumerState<PersonalChatContact> {
...
class SelectedContact extends ConsumerStatefulWidget {
 final  id;
  ...
  // bool reload;
   SelectedContact({ Key? key, this.image,this.name,this.chatitems,
...
Future getonetoonConnectionResponseData(ChatData chatdata)async { 
    onetoonConnectionResponse = await GroupAndChatRepository().getonetooneConnectionResponse(id: widget.id);
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      child: InkWell(
        ...
        onTap: ()async{
          // for (var i = 0; i < widget.chatitems!.length; i++) {
          //   chatprov
          // }
          chatprov.id=widget.id;
          chatprov.image=widget.image;
          chatprov.name=widget.name;
          int id=
         ...
         //  Navigator.push(context, MaterialPageRoute(builder: (context) => PersonalChat()))
          pushNewScreen(
          context,
          screen:  PersonalChat(connectionId: id ,) ,
          withNavBar: false, // OPTIONAL VALUE. True by default.
          pageTransitionAnimation: PageTransitionAnimation.cupertino,
      );
        },
