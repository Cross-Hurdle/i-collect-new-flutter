...
import 'package:icollekt/Model/ProductCommentUnLikeModle.dart';
import 'package:icollekt/Services/Provider.dart';

class ProductComment extends ConsumerStatefulWidget {
  final int? id;
  const ProductComment( {Key? key,required this.id,}) : super(key: key);

  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _ProductCommentState();
}
...
class _ProductCommentState extends ConsumerState<ProductComment> {
 ...
 createcomment() async {
    ...
    GroupAndChatRepository().getcreateProductcommentResponse(
      postid:widget.id, 
     // groupid:ref.read(viewgroupprov).groupid! ,
       comment:_controller.text.toString() );
       print('hiiiiiiiiiii   ${ref.read(commentinfo).id!}');
     if(createcommentResponse['result']==true){
       
      _controller.clear();
      updatecomment();
       
     }   else if(createcommentResponse['result']==false){
   ...
   ProductCommentModle? commentResponse;
   Future updatecomment() async {
      // print('hiiiiiiiiiii   ${ref.read(commentinfo).id!}');
   commentResponse = await GroupAndChatRepository().getProductcommentResponse(postid:widget.id,);
   status=true;
      // print('hiiiiiiiiiii   ${ref.read(commentinfo).id!}');
   ...
   return 
           commentResponse!.userData.isNotEmpty?    Expanded(
            child: 
                StreamBuilder(
                  stream: 
                   updatecomment().asStream(),
                  builder: (context, snapshot) {
                    return ListView.builder(
                      
                      reverse:false,
                      itemCount: commentResponse?.userData.length??0,
                            itemBuilder: (context,index){
                               if (commentResponse!.userData[index].productsLikeComments.isNotEmpty) {
      count=commentResponse!.userData[index].productsLikeComments[0].likecount;
      commentResponse!.userData[index].productsLikeComments.forEach((element) {
      ...
      isliked=false;
                    ...
                    // appbar=true;
                     ...
                     likelist: commentResponse!.userData[index].productsLikeComments,
...
class ProductCommentContainer extends ConsumerStatefulWidget {
  final postid;
  final bool status;
  final int index;
 ...
 final  String profilepic;
  ...
  final int listcount;
  final  DateTime  time;
  ProductCommentContainer({ Key? key,required this.postid, required this.index, required this.commentid, required this.listcount,required this.status, required this.commentcolor,required this.comment,required this.likelist,required this.profilename,required this.profilepic,required this.time }) : super(key: key);

  @override
  _ProductCommentContainerState createState() => _ProductCommentContainerState();
}

class _ProductCommentContainerState extends ConsumerState<ProductCommentContainer> {
   bool isliked = false;
   int count =0;
 ...
 return "just now";
    ...
    ProductCommentLikeModle ?allpostlikeresponse;
 getallLikeresponse()async{
 allpostlikeresponse=   await AuthRepository().getProductCommentlikeData(
...
//LikeDataResponse? 
ProductCommentUnLikeModle? allUnpostlikeresponse;
 getallUnLikeresponse()async{
  // postresponse!.userData.forEach((element) {
  //     ref.read(viewgroupprov).postid=  element.id;
  //   }); 
 allUnpostlikeresponse=  await AuthRepository().getProductCommentUnlikeData(
