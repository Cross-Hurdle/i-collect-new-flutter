...
class TopMonthcollekt extends StatefulWidget {
  String? filter;
   TopMonthcollekt({ Key? key,required this.filter }) : super(key: key);

  @override
  State<TopMonthcollekt> createState() => _TopMonthcollektState();
}

class _TopMonthcollektState extends State<TopMonthcollekt> {
  @override
  void initState() { 
    super.initState();
    topcollection();
    ...
    bool status= false;
  TopMonthresponse? collectionResponse;
  topcollection() async {
     
   collectionResponse = await GroupAndChatRepository().getMonthResponse();
   status =true;
 ...
 Widget collections() {
     if (!status) {
               ...
               return 
                collectionResponse!.topmonth.length!=0? Expanded(
      child: Container(
      ...
      child:  GridView.builder(
         ...
         itemBuilder: (context, index) =>  TopCollectionmonth(
           id: collectionResponse!.topmonth[index].id,
            title: collectionResponse!.topmonth[index].name,
            description: collectionResponse!.topmonth[index].description,
            image: collectionResponse!.topmonth[index].thumbnailImg,
            price: collectionResponse!.topmonth[index].unitPrice,
          ),
         itemCount:  collectionResponse!.topmonth.length,
        ),
      ),
    ):Container(
              height: 50.h,
              child: Center(child: Text('No TopMonth Collection')));
  }
...
class TopCollectionmonth extends StatefulWidget {
 final String? image;
  final String ?title;
  final String? description;
  final int? price;
  final int? id;
  const TopCollectionmonth({ Key? key,this.description,this.image,this.price,this.title,this.id }) : super(key: key);

  @override
  State<TopCollectionmonth> createState() => _TopCollectionmonthState();
}

class _TopCollectionmonthState extends State<TopCollectionmonth> {
