...
class ProfilePostComment extends ConsumerStatefulWidget {
  const ProfilePostComment({Key? key}) : super(key: key);

  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _ProfilePostCommentState();
}

class _ProfilePostCommentState extends ConsumerState<ProfilePostComment> {
  bool status = false;
  bool isliked = false;
  int count = 0;
  ...
  bool appbar = false;
  createcomment() async {
    final createcommentResponse = await GroupAndChatRepository()
  ...
  Future updatecomment() async {
    print('hiiiiiiiiiii   ${ref.read(commentinfo).id!}');
    commentResponse = await GroupAndChatRepository().getcommentResponse(
      postid: ref.read(commentinfo).id!,
    );
    status = true;
    print('hiiiiiiiiiii   ${ref.read(commentinfo).id!}');
  ...
  commentlist() {
    ...
    if (!status) {
      ...
      return commentResponse!.userData.isNotEmpty
          ? Expanded(
              child: StreamBuilder(
                  ...
                  builder: (context, snapshot) {
                    return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          if (commentResponse!
                              .userData[index].likeComments.isNotEmpty) {
                            count = commentResponse!
                                .userData[index].likeComments[0].likecount;
                            commentResponse!.userData[index].likeComments
                                .forEach((element) {
                              if (element.userId ==
                          ...
                          return Container(
                              ...
                              child: GestureDetector(
                                  ...
                                  child: CommentContainer(
                                    index: index,
                                    commentid:
                                        commentResponse!.userData[index].id,
                                    status: isliked,
                                    listcount: count,
                                    commentcolor: appbar,
                                    profilename: commentResponse!
                                        .userData[index].user.name,
                                    profilepic: commentResponse!
                                        .userData[index].user.profilePic,
                                    comment: commentResponse!
                                        .userData[index].comment,
                                    likelist: commentResponse!
                                        .userData[index].likeComments,
